...
parser.add_argument(
    "-x", "--xl_file", type=str, required=True, help="Path to the XL file"
)
parser.add_argument(
    "-m", "--mode", type=str, default="prot_details", help="prot_names or mapping"
)
parser.add_argument(
    "-p",
    "--poi",
    type=str,
    default=None,
    help="File containing the names of proteins of interest",
)
...
def get_all_proteins_from_xl_and_fetch_pnames(xls_df) -> list[tuple[str, str]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(
        max_workers=4
    ) as executor:  #! Do not increase the max_workers parameter value, as higher values return HTTP error 429 on Entrez
        prot_details = []
        failed_searches = []
...
def get_sequences(prot_ids):
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        sequences = []
        for prot_seq in executor.map(
            uniprot_helper.get_sequence, list(prot_ids.keys())
        ):
...
if __name__ == "__main__":
    ...
    if todo == "prot_details":
        print(
            "From the above output, add the names of proteins of interest to a file and rerun this script with mode=mapping"
        )
        exit()
        ...
        with open("S2_mapping.yaml", "w") as umf:
