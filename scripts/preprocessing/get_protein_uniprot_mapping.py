...
def get_all_proteins_from_xl_and_fetch_pnames(xls_df) -> list[tuple[str, str]]:
    all_proteins = uniprot_helper.get_unique_uniprot_ids(xls_df)
    with concurrent.futures.ProcessPoolExecutor() as executor:
        protein_details = []
        for pdeet in executor.map(uniprot_helper.get_protein_name, all_proteins):
            protein_details.append(pdeet)
    print([i[1] for i in protein_details])
...
protein_details = get_all_proteins_from_xl_and_fetch_pnames(df)
